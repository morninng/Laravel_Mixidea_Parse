...
AfterDebateTab.prototype.onEnterLinkInput = function(){
  ...
  self.visible_link_loading(true);
	...
	Parse.Cloud.run('OgpRetrieve', data_sent, {
    ...
    success: function(obj) {
    	...
    	for(var i=0; i<url_ogp_array.length; i++ ){
    		...
    		if(exist == false){
    			...
    			var img_src = url_ogp_array[i].get("image");
    			/*
    			if(img_src){
    				img_src = remove_prefix(img_src);
    			}
    			*/
    			obj["img_src"] = img_src;
...
function remove_prefix(url){

	var splited_array = url.split(":");
	var removed_url = url;

	switch(splited_array[0]){
		case "http":
			removed_url = url.substr(5);
		break;
		case "https":
			removed_url = url.substr(6);
		break;
	}
	return removed_url;
}
